...
Meteor.methods({
  ...
  updateGrat : function ( originalValue, newValue ) {
    Meteor.users.update ( { "_id" : Meteor.userId(), "profile.grats.grat" : originalValue }, 
      { $set: 
        { "profile.grats.$.grat" : newValue } 
      } 
    );
  },

  deleteGrat : function ( originalValue ) {
    Meteor.users.update ( { "_id" : Meteor.userId() }, 
      { $pull: 
        { "profile.grats": originalValue } 
      } 
    );
  },

  newResolution : function ( resolution ) {
    Meteor.users.update ( { "_id" : Meteor.userId() }, 
      { $push: 
  ...
  updateResolution : function ( originalValue, newValue ) {
    Meteor.users.update ( { "_id" : Meteor.userId(), "profile.resolution" : originalValue }, 
      { $set: 
        { "profile.resolution.$" : newValue } 
      } 
    );
  },

  deleteResolution : function ( originalValue ) {
    Meteor.users.update ( { "_id" : Meteor.userId() }, 
      { $pull: 
        { "profile.resolution": originalValue } 
      } 
    );
  }
})
